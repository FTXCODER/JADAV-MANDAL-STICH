...
# ---------------- PAGE ---------------- #
st.set_page_config(page_title="Jadav Mandal Tasks", layout="wide")
...
jadav_sheet = sheet.worksheet("Jadav Mandal")
update_sheet = sheet.worksheet("TASK UPDATE")
...
@st.cache_data(ttl=60)
def load_submitted():
    data = update_sheet.get_all_values()
    if len(data) < 2:
        return set()
    df = pd.DataFrame(data[1:], columns=data[0])
    return set(df.iloc[:, 1].astype(str))  # Column B
